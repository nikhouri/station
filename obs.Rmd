...
query <- 'pctile:{ y (100 xrank y:asc y) bin x}; select last time, now:last data, low:pctile[5;data], median:med data, high:pctile[95;data] by sym,units from obs where host=`garden,sym in `temperature`humidity`pressure'
...
dfn$median <- numfmt(round(dfn$median,1))
dfn$high <- numfmt(round(dfn$high,1))
dfn$time = as.POSIXct(dfn$time,format='0D%H:%M:%S',tz='UTC')
colnames(dfn) <- c("Variable","Units","Timestamp (UTC)","Last","Min (5th%)","Median","Max (95th%)")
